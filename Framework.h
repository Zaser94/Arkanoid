...
/*  FRAMEWORK BUGS DETECTED:

    - After calling againg Init() for reset the game getTickCount() returns wrong high values, so the abilities wait too long before spawn again
	- Passing getTickCount() + unsigned int value as a function parameter produce a wrong result 
	  Specific example: make_pair(getTickCount()+ModifierDuration, strModifier->Modifier) -> [15499 +10000 = 43024]
